...
class ATTime
{
    ...
    void createRecvEvent(__uint64_t msgLogicalTime, __uint64_t msgPhysicalTime, char *recvString, ATTime* f);
...
void writeState(FILE *fp, int type, char *recvString = NULL)
{
    ...
    switch(type)
    {
        case 0: // send event
            fprintf (fp, "Send:");
            fprintf (fp, "%s:%lu:%lu:%s\n",g_myID, g_attime.mLogicalTime, g_attime.mPhysicalTime, offset);
            break;

        case 1: // recv event
            fprintf (fp, "Recv:");
            fprintf (fp, "%s:%lu:%lu",g_myID, g_attime.mLogicalTime, g_attime.mPhysicalTime);
            fprintf (fp, ":%s\n",  recvString);
            break;
...
void ATTime::createRecvEvent(__uint64_t msgLogicalTime, __uint64_t msgPhysicalTime, char *recvString, ATTime *f)
{
    ATTime *e = &g_attime;
        ...
        else if (err)
        {
            for (i = 0; i < g_peerCount; i++)
            {
                if(FD_ISSET(g_peerFds[i], &rfds))
                {
                    ...
                    g_attime.createRecvEvent(LogClk, PhyTime, buffercopy, f);
...
int main (int argc, char* argv[])
{
    ...
    while (1)
    {
        for (int i = 0; i < g_peerCount; i++)
        {
            ...
            sprintf(message, "%s:%ld:%ld:%s", g_myID, g_attime.mLogicalTime, g_attime.mPhysicalTime, offset);
            pthread_mutex_unlock(&g_lock_lc);
